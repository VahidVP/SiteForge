  ...
  return (
    <main className="container page">
      ...
      <div className="grid grid-2" style={{ gap: 28, alignItems: 'start' }}>
        <div>
          <div className="card" style={{ padding: 0, overflow: 'hidden', cursor: mainSrc ? 'zoom-in' : 'default', position: 'relative' }}>
            {mainSrc ? (
              <img src={mainSrc} alt={name} style={{ width: '100%', aspectRatio: '4 / 3', objectFit: 'cover' }}
                onClick={() => mainSrc && setZoom(true)} />
            ) : (
              <div style={{ height: 340, display: 'grid', placeItems: 'center', color: 'var(--muted)' }}>No image</div>
            )}
            {mainSrc ? (
              <button type="button" className="zoom-badge" onClick={() => setZoom(true)} aria-label={lang === 'fa' ? 'بزرگ‌نمایی' : 'Zoom'}>⤢</button>
            ) : null}
          </div>
          {allImages.length > 1 ? (
      ...
      {zoom && mainSrc ? <Lightbox src={mainSrc} alt={name} onClose={() => setZoom(false)} /> : null}
    </main>
  )
}
