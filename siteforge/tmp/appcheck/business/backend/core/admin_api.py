...
class AdminServicesView(APIView):
    ...
    def get(self, request):
        return Response(ServiceSerializer(Service.objects.order_by("order", "id"), many=True).data)
...
class AdminServiceView(APIView):
    ...
    def delete(self, request, pk):
        deleted, _ = Service.objects.filter(pk=pk).delete()
        if not deleted:
            return Response({"detail": "Not found."}, status=404)
...
urlpatterns = [
    ...
    path("services/<int:pk>/", AdminServiceView.as_view()),
